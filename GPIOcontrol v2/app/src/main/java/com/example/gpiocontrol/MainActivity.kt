    ...
    private fun initUart() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                serialPort = SerialPort(File("/dev/ttyS1"), 19200)
                uartOutput = serialPort?.getOutputStream()
                uartInput = serialPort?.getInputStream()

                Log.d("UART", "UART initialized on /dev/ttyS1 at 115200 baud")

                // Launch a coroutine to continuously read data from UART.
                launch {
                    readUartData()
                }
            } catch (e: Exception) {
